package com.technologies.gimick.stories.uiviews;

/*
This is class for placing stories in stack. When user flip one story another story will come at the
foreground and previous story will go to the background. User can also flip back to the previous story.
 ...
 */
public class StackViewVertical extends AdapterViewAnimator {
    ...
    private static final float PERSPECTIVE_SHIFT_FACTOR_X = 0.00f;
    ...
    private static final float PERSPECTIVE_SCALE_FACTOR = 0.03f;
    ...
    private final WindowManager WINDOWS_MANAGER = (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);
    private final DisplayMetrics DISPLAY_METRICS = new DisplayMetrics();

    /**
     * {@inheritDoc}
     */
    public StackViewVertical(Context context) {
        this(context, null);
    }
     ...
     * {@inheritDoc}
     */
    public StackViewVertical(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.stackViewStyleHorizontal);
    }

    /**
     * {@inheritDoc}
     */
    public StackViewVertical(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        WINDOWS_MANAGER.getDefaultDisplay().getMetrics(DISPLAY_METRICS);
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.StackViewHorizontal, defStyleAttr, 0);

        mResOutColor = a.getColor(
                R.styleable.StackViewHorizontal_resOutColorHorizontal, 0);
        mClickColor = a.getColor(
                R.styleable.StackViewHorizontal_clickColorHorizontal, 0);
    ...
    private void initStackView() {
        ...
        if (sHolographicHelper == null) {
            sHolographicHelper = new HolographicHelper(getContext());
        }
    ...
    private void transformViewAtIndex(int index, final View view, boolean animate) {
        ...
        if (mStackMode == ITEMS_SLIDE_DOWN) {
            index = getNumActiveViews() - index - 1;
            if (index == getNumActiveViews() - 1) index--;
        } else {
            index--;
            if (index < 0) index++;
        }

        float r = (index * 1f) / (getNumActiveViews() - 2);

        float scale = 1f - PERSPECTIVE_SCALE_FACTOR * (1 - r);
...
//            }
    ...
    private void beginGestureIfNeeded(float deltaY) {
        if ((int) Math.abs(deltaY) > mTouchSlop && mSwipeGestureType == GESTURE_NONE) {
            ...
            final int swipeGestureTypeY = deltaY < 0 ? GESTURE_SLIDE_UP : GESTURE_SLIDE_DOWN;
            ...
            if (mStackMode != ITEMS_SLIDE_UP) {
                activeIndex = (swipeGestureTypeY == GESTURE_SLIDE_DOWN) ? 0 : 1;
            } else {
                activeIndex = (swipeGestureTypeY == GESTURE_SLIDE_DOWN) ? 1 : 0;
            }

            boolean endOfStack = mLoopViews && adapterCount == 1 &&
                    ((mStackMode == ITEMS_SLIDE_UP && swipeGestureTypeY == GESTURE_SLIDE_UP) ||
            ...
            mSwipeGestureType = swipeGestureTypeY;
            cancelHandleClick();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                float rx = deltaX / (mSlideAmount * 0.5f);

                if (mSwipeGestureType == GESTURE_SLIDE_DOWN) {
                    float r = (deltaX - mTouchSlop * 0.5f) / mSlideAmount * 0.5f;

                    if (mStackMode == ITEMS_SLIDE_DOWN) r = 1 - r;
                    mStackSlider.setYProgress(r);
//                    mStackSlider.setXProgress(rx);
//                    mStackSlider.setYProgress(rx);
//                    mStackSlider.setXProgress(1 - r);
                    return true;
...
//                onSecondaryPointerUp(ev);
    ...
    private class StackSlider {
        ...
        private float highlightAlphaInterpolator(float r) {
            float pivot = 0.3f;
            if (r < pivot) {
                return 0.45f * cubic(r / pivot);
            } else {
                return 0.45f * cubic(1 - (r - pivot) / (1 - pivot));
            }
        }
...
//                    highlightLp.setVerticalOffset(Math.round(-r * stackDirection * mSlideAmount));
                    viewLp.setHorizontalOffset(Math.round(-r * stackDirection * mSlideAmount));
                    highlightLp.setHorizontalOffset(Math.round(-r * stackDirection * mSlideAmount));
        ...
        public void setXProgress(float r) {
            ...
            r *= 0.2f;
            viewLp.setVerticalOffset(Math.round(r * mSlideAmount));
            highlightLp.setVerticalOffset(Math.round(r * mSlideAmount));
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            child.layout(getPaddingLeft() + lp.horizontalOffset, getPaddingTop() + lp.verticalOffset,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // this following commented code is for setting the height
       /* int widthSpecSize = View.MeasureSpec.getSize(widthMeasureSpec);
        ;
        int heightInPX = (int) (DISPLAY_METRICS.heightPixels / 1.1f) *//*//* 3*//*;
        int heightSpecSize = View.MeasureSpec.makeMeasureSpec(heightInPX, View.MeasureSpec.AT_MOST);
        final int widthSpecMode = View.MeasureSpec.getMode(widthMeasureSpec);
        final int heightSpecMode = View.MeasureSpec.makeMeasureSpec(heightInPX, View.MeasureSpec.AT_MOST);
        ...
        if (heightSpecMode == View.MeasureSpec.UNSPECIFIED) {
            ...
            if (haveChildRefSize) {
                ...
                if (height <= heightSpecSize) {
                    ...
                    heightSpecSize |= View.MEASURED_STATE_TOO_SMALL;
